...
- HTTP默认端口为80 HTTPS默认端口号为443
    ...
    由于TCP/TP规范，一开始制定的时候，没有考虑到HTTP加密的原因，所以当提出的时候，只能在应用层增加一个模块也就是上图中写道的`TLS、SSL`
...
1. 浏览器请求，先去查询是否命中到强缓存（cache-control,expires），如果没有命中，那么直接就启动协商缓存
2. 命中了强缓存，就直接访问本地缓存，请求结束
3. 没有命中，首先判断Etag是否匹配，匹配了，则向服务器请求得时候带上If-None-Match得请求头。最后经历服务器决策返回304重定向或者200，304让浏览器重新定向读取缓存得内容，200得话，经历缓存协商之后进行展示
...
图上介绍了HTTP2和HTTP1在文件传输上的不一样。

- HTTP1在进行传输的时候如果客户端用了多请求来传输多个文件，同时服务器也要用多个任务要承接多个请求，这样做，对服务器的压力非常大（因为HTTP是串行的）
- HTTP2在传输的时候，客户端用单个请求来并发传输文件，他把文件切成一小块一小块，比如文件1，文件2，文件3，他会，先传输文件1的一小块，然后文件2的一小块，一次类推，直到传输完成。（而这就是多路复用，一个网络连接实现并行请求）

**在实际分析上找到HTTP1和HTTP2的差别**

![](https://github.com/4lQuiorrA/FE_Journey/blob/master/image/http/fenxihttp2.png)

我们可以看到在这个请求头上的一些不同的报头，这些就叫做伪头

- :authority 目标url模式部分（请求头）
- :method 目标url模式部分（请求头）
- :path 目标URL的路径和查询部分（绝对路径产生式和一个跟着`?`字符的查询产生式）
- :scheme 目标url模式部分（请求头）
- :status 响应头中的HTTP状态码部分（响应）

![](https://github.com/4lQuiorrA/FE_Journey/blob/master/image/http/weitou.png)

**伪头**字段是HTTP2内置的几个特殊的以`:`开头的key，用于替代HTTP/1.x中请求行/响应行中的信息，比如请求方法，响应状态码

在浏览器向服务器发送请求的时候，服务器返回带有伪头的响应，然后和服务器进行协议协商，如果浏览器支持HTTP2的话，浏览器会强制升级到HTTP2。
